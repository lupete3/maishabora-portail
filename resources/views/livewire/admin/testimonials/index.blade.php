<div>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">{{ __('Testimonials') }}</h5>
            <button wire:click="create()" class="btn btn-primary">{{ __('Create New Testimonial') }}</button>
        </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($testimonials as $testimonial)
                            <tr>
                                ...
                                <td>
                                    <button wire:click="edit({{ $testimonial->id }})" class="btn btn-sm btn-primary">{{ __('Edit') }}</button>
                                    <button wire:click="delete({{ $testimonial->id }})" class="btn btn-sm btn-danger">{{ __('Delete') }}</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
